...
-(NSString*) getZhaopinHttpJson{
    ...
    str = [NSString stringWithFormat:@"aac002=%@&awb004=%@&id=%@",_shenqingName,_shenqingPwd,[self getZhiweiID]];
...
-(void) setShenqingName:(NSString *)shenqingName
{
    _shenqingName = [shenqingName copy];
}
-(NSString*) getShenqingName
{
    return _shenqingName;
}
-(void) setShenqingPwd:(NSString *)shenqingPwd
{
    _shenqingPwd = [shenqingPwd copy];
}
-(NSString*) getShenqingPwd
{
    return _shenqingPwd;
}
...
- (IBAction)zhiweishenqingButtonClick:(id)sender {
    ...
    if([userDefaults stringForKey:@"loginUserName"].length >0){ //存储账号不为空
        
        [self setShenqingName:[userDefaults stringForKey:@"loginUserName"]];
        [self setShenqingPwd:[userDefaults stringForKey:@"loginUserPwd"]];
        dispatch_queue_t queue =
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        dispatch_async(queue, ^{
            ...
            dispatch_group_t group =dispatch_group_create();
            dispatch_group_async(group, queue, ^{
                httpString = [self getZhaopinHttpJson];
            });
            dispatch_group_async(group, queue, ^{
                dispatch_async(dispatch_get_main_queue(), ^{
...
- (IBAction)cleanLoginSfzhAndPwd:(id)sender {
    ...
    if([userDefaults boolForKey:@"isMemberLoginSfzhAndPwd"]){//如果之前已经记住了身份证号和姓名
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"提示信息" message:@"确定要清除求职招聘的账户信息？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"立即清除", nil];
        [alert show];
    }else {
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"提示信息" message:@"尚未登录" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alert show];
    }
}
...
-(void) showDialog:(NSString*)showMessage
{
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"申请结果" message:showMessage delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil ];
    [alert show];
}
@end
